...
class Page:
    ...
    def verify_text(self, expected_text, *locator):
        actual_result = self.driver.find_elements(*locator).text
        assert expected_text == actual_result, f'Expected {expected_text} but got actual {actual_result}'

    def wait_for_element_click(self, *locator):
        element = self.wait.until(EC.element_to_be_clickable(locator), message=f'Element not clickable by {locator}')
        element.click()
