...
class Header(Page):
    ...
    CURESKIN_SEARCH = (By.XPATH, "//search-modal[@class='header__search']")

    def click_search_icon(self):
        # icon = self.wait_for_element_appear(*self.CURESKIN_SEARCH_ICON)
        # actions = ActionChains(self.driver)
        # actions.move_to_element(icon).click().perform()
        self.wait_for_element_appear(*self.CURESKIN_SEARCH)
        btns = self.find_elements(*self.CURESKIN_SEARCH)
        btns[1].click()
